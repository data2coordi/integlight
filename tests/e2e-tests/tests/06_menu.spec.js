...
/****************************************************************************************:     */
/****************************************************************************************:     */
test("PC-01: メインメニュー → サブメニュー → サブサブメニューの開閉と閉じる確認", async ({
  page,
}) => {
  console.log(
    "[06_menu.spec.js] ===== START: PC-01: メインメニュー → サブメニュー → サブサブメニューの開閉と閉じる確認 ====="
  );
  // 1. ページを開く
  ...
  if (!mainBox)
    throw new Error("メインメニューリンクのboundingBoxが取得できません");
  await mainLink.click({
    position: { x: mainBox.width - 10, y: mainBox.height / 2 },
  });
  ...
  if (!subSubBox)
    throw new Error("サブサブメニューリンクのboundingBoxが取得できません");
  await subSubLink.click({
    position: { x: subSubBox.width - 10, y: subSubBox.height / 2 },
  });
...
test("モバイルでハンバーガーメニューの開閉ができる", async ({ page }) => {
  console.log(
    "[06_menu.spec.js] ===== START: モバイルでハンバーガーメニューの開閉ができる ====="
  );
  // モバイル表示に設定
...
test("SP-01: メインメニュー → サブメニュー → サブサブメニューの開閉", async ({
  page,
}) => {
  console.log(
    "[06_menu.spec.js] ===== START: SP-01: メインメニュー → サブメニュー → サブサブメニューの開閉 ====="
  );
...
test("PC-02: Tabでメイン→サブ→サブサブ開いて、ESCで全閉じ確認", async ({
  page,
}) => {
  console.log(
    "[06_menu.spec.js] ===== START: PC-02: Tabでメイン→サブ→サブサブ開いて、ESCで全閉じ確認 ====="
  );
  // 1. ページを開く
  await page.goto("https://wpdev.auroralab-design.com/", {
    waitUntil: "networkidle",
  ...
  const mainLink = page
    .locator(".main-navigation .menu-item-has-children > a")
    .first();
...
test("PC-03: Tabで開いて Shift+Tabで戻りつつメニューが階層的に閉じる確認", async ({
  page,
}) => {
  console.log(
    "[06_menu.spec.js] ===== START: PC-03: Tabで開いて Shift+Tabで戻りつつメニューが階層的に閉じる確認 ====="
  );
  // 1. ページを開く
  await page.goto("https://wpdev.auroralab-design.com/", {
    waitUntil: "networkidle",
...
test("PC-04: Tabでメイン→サブ→サブサブ→次のサブ", async ({ page }) => {
  console.log(
    "[06_menu.spec.js] ===== START: PC-04: Tabでメイン→サブ→サブサブ→次のサブ ====="
  );
  // 1. ページを開く
  await page.goto("https://wpdev.auroralab-design.com/", {
    waitUntil: "networkidle",
...
/****************************************************************************************:     */
/****************************************************************************************:     */
test("モバイルでハンバーガーメニューをキーボード操作で開閉できる", async ({
  page,
}) => {
  console.log(
    "[06_menu.spec.js] ===== START: モバイルでハンバーガーメニューをキーボード操作で開閉できる ====="
  );
  // モバイル表示に設定
  await page.setViewportSize({ width: 375, height: 800 });
...
test("SP-02: モバイルでTabでメイン→サブ→サブサブ開いて、ESCで全閉じ確認", async ({
  page,
}) => {
  console.log(
    "[06_menu.spec.js] ===== START: SP-02: モバイルでTabでメイン→サブ→サブサブ開いて、ESCで全閉じ確認 ====="
  );
  // 1. モバイル表示に設定してページを開く
  await page.setViewportSize({ width: 375, height: 800 });
  await page.goto("https://wpdev.auroralab-design.com/", {
...
test("SP-03: Tabで開いて Shift+Tabで戻りつつメニューが階層的に閉じる確認", async ({
  page,
}) => {
  console.log(
    "[06_menu.spec.js] ===== START: SP-03: Tabで開いて Shift+Tabで戻りつつメニューが階層的に閉じる確認 ====="
  );
  // 1. モバイル表示に設定してページを開く
  await page.setViewportSize({ width: 375, height: 800 });
  await page.goto("https://wpdev.auroralab-design.com/", {
...
/****************************************************************************************/
test("アクセシビリティ: モバイルハンバーガーのaria-expanded/aria-hidden切替検証", async ({
  page,
}) => {
  console.log(
    "[06_menu.spec.js] ===== START: アクセシビリティ: モバイルハンバーガーのaria-expanded/aria-hidden切替検証 ====="
  );
  // モバイル表示に設定
  await page.setViewportSize({ width: 375, height: 800 });
  await page.goto("https://wpdev.auroralab-design.com/", {

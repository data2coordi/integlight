...
// 3. メインクエリを投稿一覧用に調整
function modify_news_archive_query($query)
{
	if (!is_admin() && $query->is_main_query() && get_query_var('is_news_archive')) {
...
add_action('pre_get_posts', 'modify_news_archive_query');

// 4. テンプレートをhome.php（投稿一覧用）に強制切り替え（必要なら）
// たいていarchive.phpやhome.phpでOKなので不要な場合はコメントアウトしてください
/*
add_action('template_redirect', function() {
    if (get_query_var('is_news_archive')) {
...
// 5. 管理者が管理画面にアクセスしたときだけ一度だけflush_rewrite_rulesを実行
function conditional_flush_rewrite_rules()
{
	if (current_user_can('manage_options') && !get_option('custom_rewrite_rules_flushed')) {
		flush_rewrite_rules();
		update_option('custom_rewrite_rules_flushed', 1);
	}
}
add_action('admin_init', 'conditional_flush_rewrite_rules');

...
class Integlight_Customizer_Manager
{
	...
	private function default_panels()
	{
		$site_desc = __(
			'サイト全体に関連する設定です。'
				. '<br><br><b>＜お勧めの設定＞</b>'
				. '<br>「ホームページ設定」は通常は既定のままで問題ありません。'
				. '<br>他の２つは設定することをお勧めします。',
			'integlight'
		);
		$menu_desc = __(
			'メニューに関連する設定です。'
				...
				. '<br><br>２．次に、下記「メニューの位置」で作成したメニューを配置する位置を設定してください。'
				. '<br>（ヘッダーとフッターの２箇所からメニューの位置を選択できます）'
				. '<br><br><b>＜お勧めの設定＞</b>'
				. '<br>よく使われるメニュー項目をヘッダーに配置し、あまり使われないメニュー項目はフッターに配置',
			'integlight'
		);
		...
		$design_desc = __(
			'サイト全体のデザインに関連する設定です。'
				. '<br><br><b>＜お勧めの設定＞</b>'
				. '<br>配色にあるアクセントカラーは設定することをお勧めします。'
				. '<br>それ以外は通常は既定のままで問題ありません。',
			'integlight'
		);
